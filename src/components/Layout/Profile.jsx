...
export default function Profile({ user, isOwnProfile }) {
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      const res = await userImageUpload({
        key: "avatar",
        imageData: formData,
      }).unwrap();
  ...
  const handleAddSkill = () => {
    ...
    setProfile((prev) => {
      ...
      return {
        ...
        skills: [
          ...prev.skills,
          { skillName: trimmedSkillName, experience: trimmedExperience },
        ],
      };
    });
  ...
  const handleRemoveSkill = (skillToRemove) => {
    setProfile((prev) => ({
      ...
      skills: prev.skills.filter(
        (skill) => skill.skillName !== skillToRemove.skillName,
      ),
    }));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-10 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage your identity within the CPCCU community
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 flex flex-col gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center">
            ...
            <p className="text-blue-600 font-semibold uppercase text-sm tracking-widest mt-1">
              CPCCU Member
            </p>
          </div>
                    ...
                    <div className="text-gray-800 font-bold text-lg truncate">
                      {field === "github" || field === "linkedin" ? (
                        profile[field] ? (
                          ...
                          >
                            {profile[field].replace(/^https?:\/\/(www\.)?/, "")}
                          </a>
                        ) : (
                          "Not linked"
                        )
      ...
      {/* Alerts */}
      {isUpdateSuccess && (
        <SuccessAlert title="Success!" text="Your profile has been updated." />
      )}
      {isUpdateError && (
        <ErrorAlert title="Error" text="Failed to save profile changes." />
      )}
      {isImageSuccess && (
        <SuccessAlert title="Success!" text="Profile picture updated." />
      )}
      {isImageError && (
        <ErrorAlert
          title="Upload Failed"
          text={uploadError?.data?.message || "Failed to upload image."}
        />
      )}
    </div>
  );
